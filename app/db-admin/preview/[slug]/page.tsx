...
export async function generateMetadata({
  ...
  if (!article) {
    return {
      title: '预览文章不存在 | 独立数据库后台',
    }
  }

  return {
    title: `${article.title} | 独立数据库预览`,
    description: article.description,
    alternates: {
      canonical: `/db-admin/preview/${article.slug}`,
    },
    robots: {
      index: false,
      follow: false,
...
export default async function DbAdminPreviewPage({
  params,
}: {
  params: Promise<{ slug: string }>
}) {
  ...
  const previewJsonLd = {
    ...
    url: absoluteUrl(`/db-admin/preview/${article.slug}`),
  }
    ...
    <main className="min-h-screen bg-background">
      ...
      <div className="border-b border-white/[0.06] bg-background/95 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-xs text-muted-foreground">数据库文章预览</div>
        </div>
      </div>
