...
export function AcademySubnav({
  ...
  return (
    <div className={cn('flex justify-start', className)}>
      <div className="grid w-full grid-cols-3 gap-1 rounded-[22px] border border-white/[0.08] bg-card/60 p-1 sm:inline-flex sm:w-auto sm:max-w-fit sm:rounded-full">
        {items.map((item) => {
          ...
          return (
            <Link
              ...
              href={item.href}
              className={cn(
                'inline-flex min-w-0 items-center justify-center gap-1 rounded-[18px] px-2 py-2 text-[11px] font-semibold leading-tight transition sm:gap-1.5 sm:rounded-full sm:px-4 sm:text-sm',
                isActive
