...
export function DbAdminArticlesPage() {
  ...
  async function handleDelete(article: InsightForm) {
    if (article.isNew) {
      ...
      setSelectedSlug((current) =>
        current === article.slug ? null : current,
      )
      return
    }
  ...
  async function handleLogout() {
    ...
    window.location.href = '/admin/login?next=/db-admin'
  }
          ...
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-3xl">
              <div className="mb-3 font-mono text-[11px] tracking-[0.18em] text-primary">
                // ISOLATED DATABASE ADMIN
              </div>
              <h1 className="font-serif text-4xl font-black text-foreground">
                独立数据库文章后台
              </h1>
              <p className="mt-4 text-sm leading-7 text-muted-foreground">
                这是单独隔离的数据库后台页，不接入当前前台页面。这里只管理文章数据库内容。
              </p>
            </div>
